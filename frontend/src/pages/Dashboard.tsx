...
import { getAqiJatim, type AqiJatim } from "../api/aqi";

export function Dashboard() {
  const { user } = useAuth();

  const [aqiData, setAqiData] = useState<AqiJatim | null>(null);
  const [loading, setLoading] = useState(true);

  // Ambil AQI rata-rata Jawa Timur saat halaman dibuka,
  // lalu perbarui tiap 2 jam mengikuti siklus collector di VPS.
  useEffect(() => {
    let cancelled = false;

    async function fetchAqi() {
      try {
        const data = await getAqiJatim();
        if (!cancelled) setAqiData(data);
      } catch (err) {
        console.error("Gagal mengambil AQI Jatim:", err);
    ...
    fetchAqi();
    const timer = setInterval(fetchAqi, 2 * 60 * 60 * 1000); // 2 jam
  ...
  const interpretationLabel = loading
    ? "Mengambil data..."
    : aqiData?.kategori ?? "Tidak diketahui";
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <h2 className="text-white">
                  {aqiData ? aqiData.location : "Jawa Timur"}
                </h2>
                {aqiData && (
                  <p className="text-blue-100 text-xs mt-1">
                    Rata-rata {aqiData.jumlahKota} kota — Interval data setiap 2 jam
                  </p>
                )}
              </div>
