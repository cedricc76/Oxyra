...
import { Wind, User, LogOut, LayoutDashboard, MessageSquare, BarChart2 } from 'lucide-react';
...
export function DashboardHeader() {
  ...
  // Tema gelap dinonaktifkan permanen (kebutuhan dokumentasi skripsi:
  // seluruh tangkapan layar harus berlatar terang).
  useEffect(() => {
      ...
      <div className="container mx-auto px-4 py-4 max-w-6xl">
        <div className="flex items-center justify-between">
          ...
          <Link to="/dashboard" className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-slate-900 dark:text-white">OXYRA</h1>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-4">
            ...
            <nav className="hidden md:flex items-center gap-2">
              ...
              {/* Tambahan: link ke halaman Charts */}
              <Link
                to="/charts"
                className="flex items-center gap-2 px-4 py-2 rounded-lg text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
              >
              ...
              {showUserMenu && (
                <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                  <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-700">
                    <p className="text-sm text-slate-900 dark:text-white">{user?.fullName}</p>
